...
const Dashboard = () => {
    ...
    if (!stats) return <div className="status-msg">Error loading dashboard</div>;

    // --- RTL-Aware Sequential Chart Logic ---
    const prepareChartData = () => {
        // 1. Ensure we have an array and sort it Oldest -> Newest
        const recent = [...(stats.quizzes?.recent || [])].sort((a, b) => 
        ...
        const getX = (i) => {
            ...
            // Standard Left-to-Right position
            let xPos = padding + (i * (width - 2 * padding)) / (recent.length - 1);
            // If Persian, flip the position (Right-to-Left)
        ...
        // 2. Map lines by matching quiz types to their sequential position
        const lines = types.map(t => {
            const filteredPoints = recent
                .map((q, i) => ({ ...q, originalIdx: i }))
                .filter(q => Number(q.type) === t.id);

            const path = filteredPoints.length > 1 
                ? filteredPoints.map((q) => `${getX(q.originalIdx)},${getY(q.score)}`).join(' ')
                : null;
        ...
        <div className="dashboard-wrapper" dir={lang === 'fa' ? 'rtl' : 'ltr'}>
            ...
            <div className="db-top-row">
                ...
                <div className="db-card main-stat highlight">
                    ...
                    <small>{strings.avg_score}: {stats.games?.avg_score?.toFixed(0) || 0}</small>
                </div>
            </div>

            {/* --- Leitner Breakdown --- */}
            <section className="db-section-card">
                <h3>{strings.leitner} Breakdown</h3>
                <div className="boxes-grid">
                    {boxes.map(box => {
                        const count = stats.words?.by_leitner?.[box.key] || 0;
                        return (
                            <div key={box.key} className="box-stat-item">
                                <div className="box-color-dot" style={{backgroundColor: box.color}}></div>
                ...
                <div className="leitner-progress-full">
                    {boxes.map(box => {
                        ...
                        return <div key={box.key} className="p-seg" style={{width: `${width}%`, backgroundColor: box.color}} />
                    })}
                </div>
            </section>
            ...
            <section className="db-section-card chart-section">
                ...
                <div className="svg-container">
                    {chartData ? (
                        <svg viewBox={`0 0 ${chartData.width} ${chartData.height}`} preserveAspectRatio="xMidYMid meet">
                            <line x1="40" y1={chartData.height - 40} x2={chartData.width - 40} y2={chartData.height - 40} stroke="#e2e8f0" strokeWidth="1" />
                            ...
                            {chartData.lines.map(line => (
                                <g key={line.id}>
                                    ...
                                    {line.data.map((q, i) => (
                                        <g key={i}>
                                            <circle cx={chartData.getX(q.originalIdx)} cy={chartData.getY(q.score)} r="5" fill={line.color} stroke="white" strokeWidth="2" />
                                            <text x={chartData.getX(q.originalIdx)} y={chartData.getY(q.score) - 12} textAnchor="middle" fontSize="10" fontWeight="bold" fill={line.color}>{q.score}%</text>
                                        </g>
                                    ))}
                                </g>
                            ))}
                ...
                <div className="chart-labels">
                    {chartData?.recent.map((q, i) => (
                        <span key={i} style={{ 
                            ...
                            {new Date(q.created_at).toLocaleDateString(lang === 'fa' ? 'fa-IR' : 'en-US', {day:'numeric', month:'short'})}
                        </span>
                    ))}
                </div>
            </section>
                    ...
                    <table className="db-table">
                        <tbody>
                            {(stats.quizzes?.recent || []).map(q => (
                                <tr key={q.quiz_id}>
                                    <td><span className="badge-score" style={{backgroundColor: q.type === 1 ? '#ebf8ff' : q.type === 2 ? '#faf5ff' : '#f0fff4'}}>{q.score}%</span></td>
                                    <td className="date-col">{new Date(q.created_at).toLocaleDateString(lang === 'fa' ? 'fa-IR' : 'en-US')}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
                        ...
                        <tbody>
                            {(stats.games?.recent || []).map(g => (
                                <tr key={g.survival_game_id}>
                                    <td className="gold-text">{g.score} pts</td>
                                    <td className="date-col">{new Date(g.created_at).toLocaleDateString(lang === 'fa' ? 'fa-IR' : 'en-US')}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
